...
SPLIT_JSON_ROOT = Path(
    os.environ.get(
        ...
        "/mnt/train-data-4-hdd/yian/freepose/baseline/splits_multi_backup_min3",
    )
)
CKPT_PATH = Path(
    os.environ.get(
...
def build_projection_gallery(batch, pred_bbox, gt_bbox):
    gallery = []
    for idx, image in enumerate(batch["images"]):
        ...
        gallery.append((canvas, f"scene {int(batch['ids'][idx])} | green=pred blue=gt"))
    return gallery
